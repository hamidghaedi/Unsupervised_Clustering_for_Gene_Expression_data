...
cc.res=ExecuteCC(clusterNum=3, ## Refer below for more details on this
                 ...
                 plot= pdf)
```
By ```clusterNum``` argument I have to provide the number of cluster that I am intrested to get. This is needed by the package ```CancerSubtypes```. The main package for performin CC in R is ```ConsensusClusterPlus``` and does not need to specify cluster number. From where I got the number 4? From inspecting the  CDF plots in the result folder (here "my/wd/UROMOL").Also I performed same analysis for datasets ```mad4k``` and ```mad6k```. So the number for clusters seems to be 4. This is not in agreement with the original [paper]( https://www.sciencedirect.com/science/article/pii/S1535610816302094), where the authors reported three subtypes. However in new report from the same group using a new pipeline they conculded that NMIBC to have four diffrent subtype! [ref](https://www.nature.com/articles/s41467-021-22465-w). 
